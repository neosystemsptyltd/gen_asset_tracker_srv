...
public:
	...
	{
		log_information("Connection from");
	...
	void ProcessIncomingMessage(void)
	{
        ...
        switch (_message[0])
        {
            case '$': // human readable text commands
                {
                    log_trace("Processing text command");
                    ProcessTextCommand();
                }
                break;
            ...
            case '#': // base91 encoded message
                {
                    log_trace("Processing base91 encoded message");
            ...
            case '@': // base64 encoded message
                {
                    log_trace("Processing base64 encoded message");
            ...
            case '&': // escaped binary message
                {
                    log_trace("Processing escaped binary message");
            ...
            default: // not understood - no response
                {
                    log_trace("Incoming message header unknown - ignored");
                }
                break;                    
        }        
	}
	...
	{		
		try
		{
			...
			if (_socket.available())
			{
				...
				{
					log_information("Connection reset by peer normally");
				}
                                _fifoIn.drain();
			}
			else
			{
				log_information("Nothing to be read - connection closed");
				delete this;
			}
		}
		catch(Poco::Net::NetException& exc)
		{
			log_information("ERROR: Exception: " + exc.displayText());
			delete this;
		}
	...
	{
		log_information("Shutdown - conn close");		
	...
	*/
	...
	{
		log_information("Error occurred");		
	}
	...
	*/
	void onSocketTimeout(const AutoPtr<TimeoutNotification>& pNf)
	{
		...
		try
		{
			...
			if (u32Timeout == 0)
			{
				log_information("Connection Timeout");		
				delete this;
			}
		}
		catch (...)
